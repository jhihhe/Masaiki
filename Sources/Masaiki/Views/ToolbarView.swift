        ...
        HStack(spacing: 16) {
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("强度").font(.caption2).foregroundColor(.secondary)
                    Slider(value: $viewModel.currentIntensity, in: 0.1...1.0).frame(width: 120)
                }
            }
            ...
            HStack(spacing: 8) {
                Button(action: {
                    if let item = viewModel.selectedItem { viewModel.save(item: item) }
                }) {
                    Label("覆盖保存", systemImage: "square.and.arrow.down")
                }
                .buttonStyle(.bordered)
                .disabled(viewModel.selectedItem == nil)

                Button(action: { viewModel.exportSelected() }) {
